...
#Importing dataset
''' variavel dependente sempre em ultima coluna no arq de entrada'''
dataset = pd.read_csv('Data.csv')

'''matriz recebera so os valores de iloc[todas_as_linhas;colunas_ate_a_penultima]'''
matrizX = dataset.iloc[:,:-1].values
'''matriz recebera so os valores de iloc[todas_as_linhas;ultima_coluna]'''
...
''' strategy=mean quer dizer que faz a media dos valores axis = 0 faz a media entre  elem das colunas, axis = 1 faz a media entre as linhas '''
...
'''
O labelencoder
    faz o seguinte: ele pega uma coluna que seus
     atributos sejam uma categoria e atribui valores 
     discretos para os elementos, ou seja converte strings 
     em um numero inteiro
O onehotencoder
    faz o seguinte: a partir do momento que se tem inteiro
    distintos representando strings ele separa uma 
    representacao desses inteiro em colunas
'''
